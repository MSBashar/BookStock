...
@section('sub_header')
    @if($role)
        <p class="text-gray-600 dark:text-gray-400">{{ __("Add a new category to the collection as an") }}
...
@section('content')
    ...
    <form method="post" action="{{ route('categories.store') }}" class="space-y-6">
        @csrf
        <div>
        <label for="name" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Category Name') }} <span class="text-red-500">*</span></label>
        <input
            type="text"
            id="name"
            name="name"
        ...
        ></textarea>
        @error('description')
            <p class="bg-red-100 rounded-lg text-red-600 mt-1 p-1 text-sm">* {{ $errors->first('description') }}</p>
        @enderror
        </div>
