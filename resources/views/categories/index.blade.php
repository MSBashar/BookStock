...
@section('sub_header')
     @if($role)
        <p class="text-gray-600 dark:text-gray-400">{{ __("Manage book categories as an") }}
                ...
                @foreach ($categories as $category)
                    <tr>
                        ...
                        @php
                            $sl++;
                        @endphp
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ __($category->name) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ __($category->description) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center">{{ __($category->active_books_count) }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-2 gap-1">
                                <a href="{{ route('categories.show', $category->id) }}" class="text-indigo-600 hover:text-indigo-800 transition-colors duration-200">
                                ...
                                @hasanyrole(['admin', 'editor'])
                                    <a href="{{ route('categories.edit', $category->id) }}" class="text-indigo-600 hover:text-indigo-800 transition-colors duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    ...
                                    @hasanyrole(['admin'])
                                        ...
                                        <form action="{{ route('categories.destroy', $category->id) }}" method="POST" class="inline-block"  onsubmit="return confirm('{{ __('Are you sure you want to delete (:name)? This action cannot be undone.', ['name' => $category->name]) }}');">
                                            @csrf
                                            @method('DELETE')
