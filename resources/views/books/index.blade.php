...
@section('sub_header')
     @if($role)
        <p class="text-gray-600 dark:text-gray-400">{{ __("Manage books as an") }}
...
@section('button')
    <a href="{{ route('books.create') }}"
        ...
        <span class="hidden sm:inline">{{ __('Add Book') }}</span>
    </a>
@endsection
@endhasanyrole
        ...
        <table class="w-full">
          <thead class="bg-gray-50 border-b">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Cover') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Title') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Author') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Category') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('ISBN') }}</th>
              <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Status') }}</th>
              <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('') }}</th>
                ...
                @foreach ($books as $book)
                    <tr>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                            <img src="{{ asset('storage/'.$book->cover_image) }}" alt="Cover Image" class="w-12 h-16 object-cover rounded-lg shadow-sm">
                            {{-- <img src="{{ Storage::url('cover_image/'.$book->cover_image) }}" alt="Cover Image" width="20%"> --}}
                        </td>
                        <td class="px-4 py-3 whitespace-normal text-sm text-gray-900">{{ __($book->title) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ __($book->author_name) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ __($book->category_name) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center ">{{ __($book->isbn) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            @switch($book->status)
                                @case('Available')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">{{ __($book->status) }}</span>
                                    @break
                                @case('Borrowed')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">{{ __($book->status) }}</span>
                                    @break
                                @default
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">{{ __($book->status) }}</span>
                            @endswitch
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            @if($book->active === 1)
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-2 gap-1">
                                <a href="{{ route('books.show', $book->id) }}" class="text-indigo-600 hover:text-indigo-800 transition-colors duration-200">
                                ...
                                @hasanyrole(['admin', 'editor'])
                                    <a href="{{ route('books.edit', $book->id) }}" class="text-indigo-600 hover:text-indigo-800 transition-colors duration-200">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    ...
                                    @hasanyrole(['admin'])
                                        ...
                                        <form action="{{ route('books.destroy', $book->id) }}" method="POST" class="inline-block"  onsubmit="return confirm('{{ __('Are you sure you want to delete (:title)? This action cannot be undone.', ['title' => $book->title]) }}');">
                                            @csrf
                                            @method('DELETE')
