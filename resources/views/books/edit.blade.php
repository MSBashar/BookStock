...
@section('sub_header')
    @if($role)
        <p class="text-gray-600 dark:text-gray-400">{{ __("You are editing the book as an") }}
...
@section('content')
    ...
    <form method="post" action="{{ route('books.update', $book) }}" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @method('put')
        ...
        <div>
            ...
            <div class="flex-shrink-0">
                <div
                id="oldImage"
                class="w-32 h-44 rounded-lg bg-gray-100 border-2 border-gray-200 flex items-center justify-center overflow-hidden div_show"
                ...
                <div
                id="imagePreview"
                class="w-32 h-44 rounded-lg bg-gray-100 border-2 border-gray-200 flex items-center justify-center overflow-hidden div_hide"
                >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-gray-300">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <input
                ...
                value="{{ old('title', $book->title) }}" placeholder="{{ __('Enter book title') }}"
            />
            @error('title')
            ...
            <input
                ...
                required
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                value="{{ old('isbn', $book->isbn) }}" placeholder="{{ __('978-0-7475-3269-9') }}"
            />
            @error('isbn')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input
                    ...
                    name="published_at"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                    value="{{ old('published_at', $localTime->format('Y-m-d')) }}"
                />
                @error('published_at')
            ...
            <div>
                ...
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                >
                <option value="Available" {{ old('status', $book->status) == "Available" ? 'selected' : '' }}>{{ __('Available') }}</option>
                <option value="Borrowed" {{ old('status', $book->status) == "Borrowed" ? 'selected' : '' }}>{{ __('Borrowed') }}</option>
                <option value="Reserved" {{ old('status', $book->status) == "Reserved" ? 'selected' : '' }}>{{ __('Reserved') }}</option>
                </select>
                @error('status')
        ...
        <div>
            ...
            name="active"
            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                <option value="1" {{ old('active', $book->active) == "1" ? 'selected' : '' }}>{{ __('Active') }}</option>
                <option value="0" {{ old('active', $book->active) == "0" ? 'selected' : '' }}>{{ __('Inactive') }}</option>
            </select>
        </div>
      ...
      function previewImage(event) {
        ...
        if (file) {
          ...
          reader.onload = function(e) {
            const old_image = document.getElementById('oldImage');
            old_image.classList.remove('div_show');
            old_image.classList.add('div_hide');
            const preview = document.getElementById('imagePreview');
            preview.classList.remove('div_hide');
            preview.classList.add('div_show');
